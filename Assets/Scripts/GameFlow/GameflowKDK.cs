...
public class GameflowKDK : GameflowBase
{
    #region Enums
    ...
    public static GameflowKDK myself => instance as GameflowKDK;
    ...
    const string MPT_ANTENNAS = "an";
    const string MPT_GOLD = "go";
    ...
    {
        Debug.Log("[STATES] GameflowKDK destroyed!");
        if (ownedobject)
        {
            ChoiceButtons.onSetChoiceCallback -= OnChoiceButtons;
            TowerDefManager.onTowerDefState -= OnTowerDefState;
    ...
    {
        return gameState >= GameState.ShowRaceWorld;
    }
                ...
                if (gameState == GameState.EndGame)
                {
                    ...
                    switch ((gamesettings.EndButtons)data)
                    {
                        ...
                        case gamesettings.EndButtons.END_NEXT_RACE:
                            ...
                            multiplayerlobby.startLevel = levelIndex;
                            multiplayerlobby.startWave = 0;
                            break;
...
//                Debug.Log("Keeping "+jsonterm+" "+ obj.gameObject.name);
    ...
    public override string OnSendingData()
    {
        ...
        {                                   // send info from the player
            ...
            if (PhotonNetwork.IsMasterClient)
            {
                ...
                if (TowerDefManager.myself != null && gameState == GameState.TowerDef)
				{
                    List<float> healthValues = TowerDefManager.myself.GetHealthToDefendValues();
                    string antennasMsg = "";
                    foreach (float v in healthValues)
                    {
                        antennasMsg += v.ToString("F3") + "_";
                    }
                    antennasMsg += TowerDefManager.myself.regeneratorLife.ToString("F3");
                    jsontext += "\"" + MPT_ANTENNAS + "\":\"" + antennasMsg + "\",";
                }

                if (gameplayrunning && Player.myplayer != null)
                    jsontext += "\"" + MPT_GOLD + "\":\"" + Player.myplayer.collectedTeamGold + "\",";
            }
                ...
                if (received != null)
                {
                    ...
                    if (received[MPT_TOWERDEFEVENTS] != null)
                    {
                        if (TowerDefManager.myself != null)
                        {
                            ...
                            {
                                if (System.Enum.TryParse(split[i], out TowerDefManager.TowerDefState state))
                                {
                                    string msgData = split[i + 1];
                                    if (int.TryParse(msgData, out int numWave))
                                    {
                                        string msgData2 = split[i + 2];
                                        if (int.TryParse(msgData, out int relativeWave))
                                        {
                    ...
                    if (received[MPT_ANTENNAS] != null)
                    {
                        if (TowerDefManager.myself != null)
                        {
                            string valString = ((string)received[MPT_ANTENNAS]);
                            string[] split = valString.Split('_');
                            List<float> values = new List<float>();
                            int messageCount = split.Length;
                            for (int i = 0; i < messageCount; ++i)
                            {
                                if (float.TryParse(split[i], out float val))
                                {
                                    if (i < messageCount - 1)
                                        values.Add(val);
                                    else
                                        TowerDefManager.myself.UpdateGeneratorLife(val);
                                }
                            }
                            TowerDefManager.myself.SetHealthToDefendValues(values);
                        }
                    }

                    if (received[MPT_GOLD] != null)
					{
                        if (Player.myplayer != null)
						{
                            string valString = ((string)received[MPT_GOLD]);
                            if (int.TryParse(valString, out int val))
                            {
                                Player.myplayer.SetCollectedTeamGold(val);
                            }
                        }
					}
        ...
        if (ownedobject)         // Only from our player
        {
            ...
            else if (gameState == GameState.NameValidated)
            {
                if (AreAllFlowsAtState(GameState.NameValidated, true))
                {
                    ...
                    levelToLaunch = null;
                    //if (StarterHub.myself != null)
                    //    StarterHub.myself.InvokeLevelButton();
                }
            }
    ...
    public static bool AreAllFlowsAtState(GameState state, bool useFind = false)
    {
        ...
        foreach (GameflowKDK flow in allFlows)
        {
    ...
    IEnumerator LoadMyScene()
    {
        ...
        else
        {
            ...
            allFlows = FindObjectsOfType<GameflowKDK>();

...
public class PlayerUpdateAim : MonoBehaviour
{
    ...
    [Header("Set angle to hand")]
    public bool showRenderers = true;
    public string[] componentsToSearchNames = null;
    public Transform trToApplyAngle = null;
    public bool isPositiveAngle = true;

    private bool _isAngleApplied = false;

    public float aimAngle => _aimAngle;
    private float _aimAngle = 39.4f;
    ...
    public void UpdateComponents(CVRRenderModels renderModels)
    {
        ...
        for (int childIndex = 0; childIndex < transform.childCount; childIndex++)
        {
            ...
            if (attach != null)
            {
                ...
                Debug.Log($"PlayerUpdateAim - test angle {attach.localRotation.eulerAngles.x} from {componentName}");
                SearchAndApplyAngle(componentName, attach);
    ...
    private void SearchAndApplyAngle(string componentName, Transform attach)
	{
        if (componentsToSearchNames != null && trToApplyAngle != null && !_isAngleApplied)
        {
            foreach (string name in componentsToSearchNames)
            {
                if (componentName == name)
                {
                    _aimAngle = PatchSpecificAngle(attach.localRotation.eulerAngles.x);
                    Debug.Log($"PlayerUpdateAim - found angle {_aimAngle} from {componentName}");
                    trToApplyAngle.localRotation = Quaternion.Euler(0f, 0f, isPositiveAngle ? _aimAngle : -_aimAngle);
                    _isAngleApplied = true;
                    return;
                }
            }
        }
    ...
    private float PatchSpecificAngle(float angle)
	{
        var system = OpenVR.System;
        var error = ETrackedPropertyError.TrackedProp_Success;
        var capacity = system.GetStringTrackedDeviceProperty((uint)index, ETrackedDeviceProperty.Prop_ControllerType_String, null, 0, ref error);
        var buffer = new System.Text.StringBuilder((int)capacity);
        system.GetStringTrackedDeviceProperty((uint)index, ETrackedDeviceProperty.Prop_ControllerType_String, buffer, capacity, ref error);
        string type = buffer.ToString();
        Debug.Log("PlayerUpdateAim - type " + type);
        if (type.Contains("vive"))
        {
            // Patch for vive pro
            Debug.Log("PlayerUpdateAim - patch vive controllers from " + angle + " to 40!");
            angle = 40f;
        }
        //if (type == "oculus_touch")
        //{
        //}
        return angle;
    }

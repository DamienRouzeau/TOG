...
public class UnityAssetXrefs {
    ...
    public static void SelectSceneUsesOfAsset() {
		...
		foreach( Object result in results )
		{
			if( result!=null )
			{
				Object prefab = PrefabUtility.GetCorrespondingObjectFromSource( result );
				string sPrefab = prefab!=null ? prefab.GetType().ToString() : "";
					Debug.Log( result.name + " / " + sPrefab, result );
			}
		}
    ...
    public static void SelectUsesOfAsset() {
		foreach (Object obj in Selection.objects)
		{
			...
			Debug.Log("You have asked which assets make use of " + cur.name + ", a " + cur.GetType().ToString() + ".", cur);
			List<Object> results = CollectReverseDependencies(cur);
			Debug.Log(results.Count + " reverse dependencies found for "+cur.name+" in " + (Time.realtimeSinceStartup - startTime).ToString() + " secs.", cur);
			printAssetPaths(results);
			setSelection(results);
		}
	}

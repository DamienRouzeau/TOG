...
public class SplineInterpolator : MonoBehaviour
{
	...
	private bool _useForwardRotNode = false;
...
//	OnEndCallback mOnEndCallback;
	...
	public float GetTimeAtNode(int idx)
	{
		if (idx >= 0 && idx < mNodes.Count)
			return mNodes[idx].Time;
		else
			return -1f;
	}
...
//		mCurrentTime = 0;
	...
	public void ForceDirectionWithNode(bool forceDirectionWithNode)
	{
		_useForwardRotNode = forceDirectionWithNode;
	}
	...
	*/
	...
	public Vector3 GetHermiteInternal(int idxFirstPoint, float t)
	{
		...
		if (_useForwardRotNode)
		{
			float magnitude = (P2 - P1).magnitude;
			Vector3 dirForward = Vector3.forward * magnitude;
			T1 = mNodes[idxFirstPoint].Rot * dirForward;
			T2 = mNodes[idxFirstPoint+1].Rot * dirForward;
		}
		else
		{
	...
	public Quaternion GetRotationInternal(int idxFirstPoint, float t)
	{
		float t2 = t * t;
		...
		return Quaternion.LookRotation(Blend1 * P1 + Blend2 * P2 + Blend3 * T1 + Blend4 * T2, Vector3.up);
	}

...
namespace RRLib
{
    ...
    /// </summary>
    public class RRLocalizationImporterTool : EditorWindow
    {
        public static string FileSelection(string sLabel, string sFilePath, string[] sFilters, RRPathUtil.PathType fileType = RRPathUtil.PathType.Any, string sBrowseWindowTitle = "Choose a file")
        {
            EditorGUILayout.BeginHorizontal(GUILayout.ExpandWidth(true));
            Vector2 labelSize = GUI.skin.label.CalcSize(new GUIContent(sFilePath));
            Vector2 buttonSize = GUI.skin.button.CalcSize(new GUIContent("Browse"));
            ...
            return EditorFileSelection(controlRect, sLabel, sFilePath, sFilters, fileType, sBrowseWindowTitle);
        }

        public static string EditorFileSelection(Rect rect, string sLabel, string sFilePath, string[] sFilters, RRPathUtil.PathType eFileType = RRPathUtil.PathType.Any, string sBrowseWindowTitle = "Choose a file")
        {
            Debug.Assert(sFilters != null && sFilters.Length > 0 && (sFilters.Length % 2) == 0);

            Vector2 v2ButtonSize = GUI.skin.button.CalcSize(new GUIContent("Browse"));
            ...
            if (GUI.Button(buttonRect, new GUIContent("Browse")))
            {
                string sAbsoluteSourceFilePath;
                string sAbsoluteSourceDirectoryPath;
                if (!string.IsNullOrEmpty(sFilePath) && RRPathUtil.ToAbsolute(eFileType, sFilePath, out sAbsoluteSourceFilePath))
                {
                ...
                {
                    string sResultPath;
                    if (RRPathUtil.ToRelative(eFileType, sSelectedAbsoluteFilePath, out sResultPath))
                    {
        ...
        public static string FolderSelection(string sLabel, string sDirectoryPath, RRPathUtil.PathType folderType = RRPathUtil.PathType.Any, string sBrowseWindowTitle = "Choose a folder", string sBrowseDefaultName = "Folder")
        {
            EditorGUILayout.BeginHorizontal(GUILayout.ExpandWidth(true));
            ...
            return EditorFolderSelection(controlRect, sLabel, sDirectoryPath, folderType, sBrowseWindowTitle, sBrowseDefaultName);
        }

        public static string EditorFolderSelection(Rect rect, string sLabel, string sDirectoryPath, RRPathUtil.PathType eFolderType = RRPathUtil.PathType.Any, string sBrowseWindowTitle = "Choose a folder", string sBrowseDefaultName = "Folder")
        {
            Vector2 v2ButtonSize = GUI.skin.button.CalcSize(new GUIContent("Browse"));
            Rect labelRect = new Rect(rect.x, rect.y, rect.width - v2ButtonSize.x, rect.height);
            Rect buttonRect = new Rect(labelRect.x + labelRect.width, rect.y, v2ButtonSize.x, v2ButtonSize.y);
            ...
            {
                string sAbsoluteSourceDirectoryPath;
                if (RRPathUtil.ToAbsolute(eFolderType, sDirectoryPath, out sAbsoluteSourceDirectoryPath) == false)
                {
                    sAbsoluteSourceDirectoryPath = Application.dataPath;
                }
                ...
                {
                    string sResultPath;
                    if (RRPathUtil.ToRelative(eFolderType, sSelectedAbsoluteDirectoryPath, out sResultPath))
                    {
        ...
        public static RRLocalizationImporterTool CreateOrGetWindow()
        {
            RRLocalizationImporterTool window = EditorWindow.GetWindow<RRLocalizationImporterTool>();
        ...
        private void OnGUI()
        {
            ...
            m_sExcelFilePath = FileSelection("Excel source", m_sExcelFilePath, new string[] { "Excel files", "xls,xlsx" }, RRPathUtil.PathType.Any, "Select source file");
        ...
        #region Private
        #region Declarations
        private static readonly string s_windowPreferencesPrefix = "Rerolled/LocalizationImporterTool/";
        ...
        private void LoadPrefs()
        {
            ...
            m_exportDestinationFileOption = RRTools.ParseEnumSafe<ExportDestinationFileOption>(EditorPrefs.GetString(s_windowPreferencesPrefix + "m_exportDestinationFileOption", ExportDestinationFileOption.OneFilePerLanguagePerSheet.ToString()), ExportDestinationFileOption.OneFilePerLanguagePerSheet);
        ...
        private void OnLanguageSelectionGUI()
        {
            ...
            m_v2LanguageSelectionScrollPosition = EditorGUILayout.BeginScrollView(m_v2LanguageSelectionScrollPosition, false, false);
            {
                ...
                {
                    RRCountry countryStructure = RRCountryCode.GetRRCountry(m_sLanguageCodes[nLanguageIndex]);
        ...
        private void OnImportPanelGUI()
        {
            ...
            m_sDestinationFolderPath = FolderSelection("Destination folder", m_sDestinationFolderPath, RRPathUtil.PathType.Asset);
            if (EditorGUI.EndChangeCheck())
            {
                if (System.IO.Directory.Exists(m_sDestinationFolderPath) == false)
                {
            ...
            {
                RRCountry countryStructure = RRCountryCode.GetRRCountry(selectedLanguageEnumerator.Current);
            ...
            else if (bHasExoticLanguage)
            {
                ...
                EditorGUI.BeginChangeCheck();
                m_sExoticCharacterFolderPath = FolderSelection("Exotic character folder", m_sExoticCharacterFolderPath, RRPathUtil.PathType.Any);
                if (EditorGUI.EndChangeCheck())
                {
                    if (System.IO.Directory.Exists(m_sExoticCharacterFolderPath) == false)
                    {
        ...
        private void DoImportTexts()
        {
            try
            {
                ...
                while (nWorksheetIndex < worksheets.Length)
                {
                    ...
                    Debug.Assert(nWorksheetIndexInExcelFile >= 0 && nWorksheetIndexInExcelFile < m_excelPackage.Workbook.Worksheets.Count);
                            ...
                            if (bHasBeenCancelled == false)
                            {
                                ...
                                if (string.IsNullOrEmpty(sTextId) == false)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        Debug.Assert(m_nFirstLanguageColumnIndexes[nWorksheetIndexInExcelFile] > 0 && m_nFirstLanguageColumnIndexes[nWorksheetIndexInExcelFile] <= worksheets[nWorksheetIndex].Dimension.Columns);
        ...
        {
            Debug.Assert(m_importStringBuilders == null);
            Debug.Assert(m_importExoticCharacters == null);
            ...
            switch (m_exportDestinationFileOption)
            {
                ...
                default: Debug.AssertFormat(false, "Invalid ExportDestinationFileOption '{0}'.", m_exportDestinationFileOption); nStringBuilderCount = 0; break;
            }
...

            {
                ...
                while (languageEnumerator.MoveNext())
                {
                    RRCountry langStructure = RRCountryCode.GetRRCountry(languageEnumerator.Current);
        ...
        private void WriteTextInTargetFile(string sWorksheetName, string sLanguageCode, string sLocalizedText, string sFallbackLocalizedText)
        {
            ...
            RRCountry langStructure = RRCountryCode.GetRRCountry(sLanguageCode);
                ...
                case ExportDestinationFileOption.OneFilePerSheetInLanguageFolder:
                    {
                        ...
                        while (worksheetEnumerator.MoveNext())
                        {
                            ...
                            while (languageEnumerator.MoveNext())
                            {
                                ...
                                {
                                    Debug.AssertFormat(m_exportDestinationFileOption == ExportDestinationFileOption.OneFilePerSheetInLanguageFolder, "Invalid ExportDestinationFileOption '{0}'.", m_exportDestinationFileOption);
                    ...
                    break;
                default: Debug.AssertFormat(false, "Invalid ExportDestinationFileOption '{0}'.", m_exportDestinationFileOption); break;
            }
        ...
        private void GetStringBuilderIndexes(string sWorksheetName, out int nFirstIndex, out int nLastIndex)
        {
            ...
            switch (m_exportDestinationFileOption)
            {
                ...
                default:
                    {
                        Debug.AssertFormat(false, "Invalid ExportDestinationFileOption '{0}'.", m_exportDestinationFileOption);
                        nFirstIndex = -1;
                        nLastIndex = -1;
                    }
                    break;
        ...
        private int GetStringBuilderIndex(string sWorksheetName, string sLanguageCode)
        {
            ...
            switch (m_exportDestinationFileOption)
            {
                ...
                case ExportDestinationFileOption.OneFilePerSheetInLanguageFolder:
                    ...
                    break;
                default: Debug.AssertFormat(false, "Invalid ExportDestinationFileOption '{0}'.", m_exportDestinationFileOption); nStringBuilderIndex = -1; break;
            }

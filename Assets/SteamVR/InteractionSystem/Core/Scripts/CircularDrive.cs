...
namespace Valve.VR.InteractionSystem
{
	...
	{
		public delegate void OnDrivingEvent(bool drive, float angle);
		...
		public float movingAngleBySecond = 0f;
		...
		private void Update()
		{
			if (movingAngleBySecond != 0f && !driving)
			{
				if (limited)
				{
					float tmpAngle = Mathf.Clamp(outAngle + movingAngleBySecond * Time.deltaTime, minAngle, maxAngle);
					if (tmpAngle != outAngle)
					{
						outAngle = tmpAngle;
						UpdateAll();
					}
				...
				{
					outAngle += movingAngleBySecond * Time.deltaTime;
					UpdateAll();
				}
			}
#if USE_TESTS
			if (Input.GetKey(KeyCode.C) && (Camera.main.transform.position - transform.position).sqrMagnitude < 10f)
			{
				if (Input.GetKeyDown(KeyCode.C))
				{
					driving = true;
					onDrivingEvent?.Invoke(driving, outAngle);
				}
				else
				{
					outAngle += 10f * Time.deltaTime;
					onDrivingEvent?.Invoke(driving, outAngle);
					UpdateAll();
				}
			}
		...
		//-------------------------------------------------
		public void SetMovingAngleBySecond(float angleBySecond)
		{
			movingAngleBySecond = angleBySecond;
		}

		//-------------------------------------------------
		public void ForceOutAngle(float angle)
		{
			outAngle = angle;
			UpdateAll();
		}
		...
		//-------------------------------------------------
		public void ForceDriving(bool drive)
		{
			driving = drive;
		}

...
public class Weapon : MonoBehaviour //Photon.Pun.MonoBehaviourPun, IPunObservable
{
    // Weapon Type
    ...
    // VR
    public bool fireThatWeaponVR = false;
    ...
    void ShowTimer(string add)
    {
        TimeSpan ts = DateTime.Now - debugtime;
//        Debug.Log("DEBUG TIME " + add + ":" + ts.TotalMilliseconds);
    }
    ...
    void CheckForUserInput()
    {
        ...
        if (type == WeaponType.Beam)
        {
            if (fireThatWeaponVR && beamHeat <= maxBeamHeat && !coolingDown)
            {
                Beam();
            }
            else
    ...
    void Fire()
    {
        ...
        for (int i = 0; i < shotPerRound; i++)
        {
            ...
            if (Physics.Raycast(ray, out hit, range))
            {
                ...
                // Damage
                Health.ChangeHealthNow(hit.collider.gameObject, -damage * gamesettings_difficulty.myself.GetBulletMultiplier());
                ...
                if (makeBulletHoles && !exception)
                {
                    ...
                    if (holes.Count == 0)   // If no usable (for this hit GameObject) bullet holes were found...
                    {
                        ...
                        sbhg = defaultsToUse[UnityEngine.Random.Range(0, defaultsToUse.Count)];
                    }
                    ...
                    {
                        // Choose a bullet hole at random from the list
                        sbhg = holes[UnityEngine.Random.Range(0, holes.Count)];
                    }
        ...
        {
            //PhotonView.Get(this).RPC("SpawnMuzzleEffects", RpcTarget.AllViaServer);
            SpawnMuzzleEffects();
        }
        ...
        if (spitShells)
        {
            //PhotonView.Get(this).RPC("SpawnSpitShells", RpcTarget.AllViaServer);
            SpawnSpitShells();
        }
    ...
    public void Launch()
    {
        ...
        for (int i = 0; i < shotPerRound; i++)
        {
            ...
            if (projectile != null)
            {
                ////// DECOMENT TO HAVE SHOOT IN NON MULTIPLAYER CASE
                ...
                //PhotonView.Get(this).RPC("NetworkFire", RpcTarget.AllViaServer, projectileSpawnSpot.position, projectileSpawnSpot.rotation);
                ShowTimer("before NetworkFire");
                NetworkFire(projectileSpawnSpot.position, projectileSpawnSpot.rotation);
            }
            else
            {
        ...
        if (makeMuzzleEffects)
        {
            GameObject muzfx = muzzleEffects[UnityEngine.Random.Range(0, muzzleEffects.Length)];
            if (muzfx != null)
  ...
  //  [PunRPC]
    public void SpawnMuzzleEffects()
    {
        //if (!transform.root.GetComponent<Player>().photonView.IsMine)
        //    return;
//        GameObject effect = PhotonNetwork.Instantiate("Flash", muzzleEffectsPosition.position, muzzleEffectsPosition.rotation);
//        effect.GetComponent<TimedObjectDestroyer>().isFromServer = true;
    }

//    [PunRPC]
    public void SpawnSpitShells()
    {
        //if (!transform.root.GetComponent<Player>().photonView.IsMine)
        //    return;
//        GameObject shellGO = PhotonNetwork.Instantiate("shell", shellSpitPosition.position, shellSpitPosition.rotation);
...
//    [PunRPC]
    public void NetworkFire(Vector3 pos, Quaternion rot)
    {
        ...
        GameObject bul = poolhelper.myself.GetNextPoolItem("bullet_pool", pos, rot,gameObject);

        // Tweak bullet with no VR case
        if (GameflowBase.isInNoVR && obj == null)
        {
            obj = gameObject;
            bul.GetComponent<Projectile>().speed = 10000f;
            bul.GetComponent<Projectile>().initialForce = 10000f;
            bul.GetComponent<Projectile>().damage = 1000f;
            bul.GetComponent<Projectile>().transform.localScale = Vector3.one;
        }

        bul.GetComponent<Projectile>().projectile_creator = obj;
        ShowTimer("after NetworkFire");
    }
    /*
...
*/
    ...
    void Beam()
    {
        ...
        do
        {
            ...
            if (Physics.Raycast(ray, out hit, range))
            {
                ...
                // Damage
                Health.ChangeHealthNow(hit.collider.gameObject, -beamPower * gamesettings_difficulty.myself.GetBulletMultiplier());
    ...
    void Recoil()
    {
        ...
        float kickBack = UnityEngine.Random.Range(recoilKickBackMin, recoilKickBackMax);

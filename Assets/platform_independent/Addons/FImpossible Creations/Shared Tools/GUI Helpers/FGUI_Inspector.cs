...
namespace FIMSpace.FEditor
{
    public static class FGUI_Inspector
    {
        ...
        public static void FoldHeaderStart(ref bool foldout, GUIContent title, GUIStyle textStyle, GUIStyle horizontalStyle, Texture icon = null, int height = 22)
        {
            if (horizontalStyle != null) GUILayout.BeginVertical(horizontalStyle);
        ...
        public static GUIStyle Style(Color bgColor, int off = -1)
        {
            ...
            if (off < 0) { if (Screen.dpi != 120) newStyle.border = new RectOffset(off, off, off, off); else if (FEditor_OneShotLog.CanDrawLog("dpi")) Debug.Log("<b>[HEY! UNITY DEVELOPER!]</b> It seems you have setted up incorrect DPI settings for unity editor. Check <b>Unity.exe -> Properties -> Compatibility -> Change DPI Settings -> Replace Scaling -> System / System (Upgraded)</b> And restart Unity Editor."); }
        ...
        public static GUIStyle Style(RectOffset padding, RectOffset margin, Color bgColor, Vector4 off, int zeroBorder = 0)
        {
            GUIStyle newStyle = new GUIStyle(GUI.skin.box);
            bool g = false;
            if (off.x < 0) { if (Screen.dpi == 120) { if (FEditor_OneShotLog.CanDrawLog("dpi")) Debug.Log("<b>[HEY! UNITY DEVELOPER!]</b> It seems you have setted up incorrect DPI settings for unity editor. Check <b>Unity.exe -> Properties -> Compatibility -> Change DPI Settings -> Replace Scaling -> System / System (Upgraded)</b> And restart Unity Editor."); } else g = true; } else g = true;

...
namespace FIMSpace.FEditor
{
    ...
    {
        public static Transform FoundAnimator;
        ...
        public static bool CheckForAnimator(GameObject root, bool needAnimatorBox = true, bool drawInactiveWarning = true, int clicksTohide = 1)
        {
            ...
            if (FoundAnimator)
            {
                Animation legacy = FoundAnimator.GetComponent<Animation>();
                Animator mec = FoundAnimator.GetComponent<Animator>();
        ...
        public static Transform SearchForParentWithAnimator(GameObject root)
        {
            Animation animation = root.GetComponentInChildren<Animation>();
            if (animation) return animation.transform;
            Animator animator = root.GetComponentInChildren<Animator>();
            if (animator) return animation.transform;

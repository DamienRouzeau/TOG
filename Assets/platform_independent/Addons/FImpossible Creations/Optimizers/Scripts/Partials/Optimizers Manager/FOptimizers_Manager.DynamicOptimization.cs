...
namespace FIMSpace.FOptimizing
{
    public partial class FOptimizers_Manager
    {
        ...
        public static void CallUpdateAll()
        {
            ...
            {
                for (int c = Get.dynamicLists[i].Count - 1; c >= 0; c--)
                    Get.CheckElement(Get.dynamicLists[i][c], c, false);
            }
        }
        ...
        {
            float dist = (optimizer.GetReferencePosition() - MainCamera.transform.position).magnitude;
            FEOptimizingDistance category = QualifyDistance(dist);
            ...
            optimizer.DynamicLODUpdate(category, dist);
        ...
        public void CheckElement(FOptimizer_Base optimizer, int index, bool full = true)
        {
            ...
            FEOptimizingDistance targetClock = QualifyDistance(distance);
        ...
        private FEOptimizingDistance QualifyDistance(float distance)
        {
            for (int i = 0; i < Distances.Length; i++)
                if (distance < Distances[i]) return (FEOptimizingDistance)i;
        ...
        public bool CameraMoved(Vector3 prePos, Quaternion preRot)
        {
            ...
            if (diff.magnitude > Mathf.Max(0.000001f, Get.MoveTreshold))
                cameraMoved = true;
            else
                cameraMoved = false;

...
namespace FIMSpace.FOptimizing
{
    ...
    public class FOptimizer_BaseEditor : Editor
    {
        ...
        public void LODFrame(FOptimizer_Base targetScript)
        {
            if (Application.isPlaying)
            {
                if (targetScript.IsCulled)
                    EditorGUILayout.BeginVertical(FEditor.FEditor_Styles.Style(FOptimizers_LODGUI.culledLODColor * new Color(1.5f, 1.5f, 1.5f, 0.325f)));
                else
                {
                    if (targetScript.TransitionPercent <= 0f)
                        EditorGUILayout.BeginVertical(FEditor.FEditor_Styles.Style(FOptimizers_LODGUI.lODColors[targetScript.CurrentLODLevel] * new Color(1f, 1f, 1f, 0.2f * (targetScript.OutOfCameraView ? 0.5f : 1f))));
                    else
                    {
                        ...
                        EditorGUILayout.BeginVertical(FEditor.FEditor_Styles.Style(c));
                    }
                }
            }
        ...
        public override void OnInspectorGUI()
        {
            ...
            EditorGUILayout.BeginVertical(FEditor.FEditor_Styles.Style(new Color(0.4f, 0.7f, 0.2f, 0.15f)));

            if (Application.isPlaying) GUI.enabled = false;

            EditorGUILayout.BeginVertical(FEditor.FEditor_Styles.Style(new Color(0.95f, 0.95f, 0.95f, 0.15f)));
        ...
        protected virtual void DefaultInspectorStack(FOptimizer_Base targetScript, bool endVert = true)
        {
            ...
            if (Application.isPlaying) GUI.enabled = false;
            EditorGUILayout.BeginVertical(FEditor.FEditor_Styles.Style(new Color(0.975f, 0.975f, 0.975f, .325f)));
            ...
            if (DrawSetup)
            {
                ...
                FEditor_Styles.DrawUILine(new Color(0.5f, 0.5f, 0.5f, 0.5f), 2, 4);
                ...
                FEditor_Styles.DrawUILine(new Color(0.5f, 0.5f, 0.5f, 0.5f), 1, 5);
        ...
        protected virtual void DrawLODSettingsStack(FOptimizer_Base targetScript)
        {
            ...
            if (selectedLOD >= 0 && selectedLOD <= targetScript.LODLevels + 1)
            {
                ...
                EditorGUILayout.BeginVertical(FEditor.FEditor_Styles.Style(frameCol * new Color(1f, 1f, 1f, 0.135f)));
                ...
                FEditor.FEditor_Styles.DrawUILine(new Color(0.5f, 0.5f, 0.5f, 0.5f), 2, 4);
        ...
        {
            EditorGUILayout.BeginVertical(FEditor.FEditor_Styles.Style(new Color(0.75f, 0.75f, 0.15f, 0.2f)));
            ...
            if (DrawToOptimize)
            {
                ...
                FEditor_Styles.DrawUILine(new Color(0.5f, 0.5f, 0.5f, 0.35f), 1, 6);
                ...
                if (lodId == scr.LODLevels) if (scr.DrawDeactivateToggle)
                    {
                        ...
                        EditorGUILayout.LabelField(new GUIContent("!", "Deactivating whole game object sometimes can be hard to compute for Unity.\nIf you experience lags when rotating camera, try to NOT disable whole game object, but only disable components."), EditorStyles.whiteBoldLabel, new GUILayoutOption[1] { GUILayout.Width(20) });
                        EditorGUILayout.EndHorizontal();
                        GUI.color = preCol;
                    }

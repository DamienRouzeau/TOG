...
public class ThrustEngine : MonoBehaviour {
			...
			if (hoverMode) {
				...
				if (Physics.Raycast (transform.position, -Vector3.up, out hit, hoverDistance + hoverSafeRange)) {
					...
					if (hit.distance < hoverDistance) {
						...
						/** NEW 2019 **/
						// float propHeight = (hoverDistance - hit.distance) / hoverDistance;
      //        			Vector3 appliedForce = Vector3.up * propHeight * maxPower;
      //        			applyThrustHover(appliedForce);
						...
						if(boundObject != null){
							...
							foreach (ThrustEngine tmpThruster in thrustersBound){
								if(tmpThruster.enable && tmpThruster.hoverMode){
									thrusterCount ++;
								}
							}
						}
	...
	//void applyThrustHover(int thrusters, float distance){
		...
		// thruster.AddForceAtPosition(transform.up * force.y, thruster.transform.position, ForceMode.Acceleration);
		/** BETA */

...
inline fixed unitySampleShadow(unityShadowCoord4 shadowCoord)
{
	...
	shadowAttenuation = UNITY_SAMPLE_SCREEN_SHADOW(_ShadowMapTexture, shadowCoord);	
	...
	if (NGSS_DENOISER_ITERATIONS > 0)
	{
		...
		for (float x = -NGSS_DENOISER_ITERATIONS; x <= NGSS_DENOISER_ITERATIONS; x++)
		{
			for (float y = -NGSS_DENOISER_ITERATIONS; y <= NGSS_DENOISER_ITERATIONS; y++)
			{
				...
				fixed sampleSM = UNITY_SAMPLE_SCREEN_SHADOW(_ShadowMapTexture, float4(shadowCoord.xy + float2(x, y) / softness, shadowCoord.zw));
...
half UnityComputeForwardShadows(float2 lightmapUV, float3 worldPos, float4 screenPos)
{
	...
	realtimeShadowAttenuation = NGSS_FRUSTUM_SHADOWS_ENABLED > 0.0 ? min(realtimeShadowAttenuation, saturate(UNITY_SAMPLE_SCREEN_SHADOW(NGSS_FrustumShadowsTexture, screenPos).r + NGSS_FRUSTUM_SHADOWS_OPACITY)) : realtimeShadowAttenuation;
#endif
#endif

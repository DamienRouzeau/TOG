...
namespace RVModules.RVUtilities
{
    public class TerrainTools : MonoBehaviour
    {
        ...
        public void CreateTerrainUnevenness()
        {
            ...
            beforeChangesHeightmap = tdata.GetHeights(0, 0, tdata.heightmapWidth, tdata.heightmapHeight);
            var newHeightmap = new float[tdata.heightmapHeight, tdata.heightmapHeight];

            for (var x = 0; x < tdata.heightmapWidth; x++)
            for (var y = 0; y < tdata.heightmapWidth; y++)
            {
